...
export class ContentViewerComponent implements OnDestroy, DoCheck {
    ...
    set setLocation(location) {
        ...
        if (location != null) {
            if (this.embeddedComponents.length !== 0) {
                this.embeddedComponents.forEach(comp => {
                    if (comp.instance.type === 'mapImages') {
                        comp.instance.setLocation = location;
                    }
    ...
    loadMedia() {
        this.module.htmlConfig.forEach(input => {
            ...
            switch (input.type) {
                ...
                case 'location':
                    if (this.activity.moduleData[label] != null) {
                        this.locations.push(this.activity.moduleData[label]);
                    } else {
    ...
    private build() {
        ...
        this.embeddedComponentFactories.forEach((factory, selector) => {
            ...
            // console.log(embeddedComponentElements);
            embeddedComponentElements.forEach(element => {
                ...
                const userLocationLabel = 'setLocation';
                ...
                if (valid) {
                    ...
                    if (this.locations.length > 0) {
                        ...
                        if (locs.length === 0) {
                            ...
                            embeddedComponent.instance[locationLabel] = locs;
                            embeddedComponent.instance[userLocationLabel] = this.location;
                        }
                    }
                    if (valid) {
    ...
    stopAllAudios() {
        ...
        this.embeddedComponents.forEach(comp => {
            if (comp.instance.type === 'audio') {
                comp.instance.stopAudio();
            }
